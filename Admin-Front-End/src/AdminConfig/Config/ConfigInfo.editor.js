...
class ConfigInfoEditor extends Component {
   constructor(props) {
      super(props);
      this.state = {};
   }
   ...
   onSave = () => {
      const { configOfInfo } = this.props;
      let dataContent = {
         idConfig: configOfInfo,
   ...
   render() {
      const { configOfInfo } = this.props;
      if (configOfInfo.idConfig === undefined) return XHTML_LOADING;
      else {
         return (
...
export default connect(null, mapDispatchToProps)(ConfigInfoEditor);
